...
// GetCachedSourceAlerts returns active, non-expired alerts for a specific source
// within radius meters of (lat, lng). Used by on-demand proxy handlers (e.g. 211)
// to check their lazy cache before calling upstream APIs.
// If category is non-empty, only records whose tags include that value are returned.
func (r *BeaconAlertRepository) GetCachedSourceAlerts(ctx context.Context, source string, lat, lng float64, radius int, category string) ([]map[string]any, error) {
	query := `
		SELECT
			id, source, beacon_type, severity, title, body,
			lat, lng, radius, image_url,
			is_official, official_source, author_handle, author_display,
			incident_status, confidence, vouch_count, report_count,
			tags, expires_at, created_at,
			ST_Distance(location, ST_SetSRID(ST_Point($2, $1), 4326)::geography) AS distance_meters
		FROM beacon_alerts
		WHERE source = $5
		  AND status = 'active'
		  AND expires_at > NOW()
		  AND ST_DWithin(location, ST_SetSRID(ST_Point($2, $1), 4326)::geography, $4)
		ORDER BY distance_meters ASC
	`
	rows, err := r.pool.Query(ctx, query, lat, lng, lng, radius, source)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var (
			id, src, beaconType, severity, title, body string
			aLat, aLng                                  float64
			aRadius                                     int
			imageURL                                    *string
			isOfficial                                  bool
			officialSource, authorHandle, authorDisplay *string
			incidentStatus                              string
			confidence                                  float64
			vouchCount, reportCount                     int
			tags                                        []string
		...
		if err := rows.Scan(
			&id, &src, &beaconType, &severity, &title, &body,
			&aLat, &aLng, &aRadius, &imageURL,
			&isOfficial, &officialSource, &authorHandle, &authorDisplay,
			&incidentStatus, &confidence, &vouchCount, &reportCount,
			&tags, &expiresAt, &createdAt, &distanceMeters,
		); err != nil {
			log.Error().Err(err).Msg("cached_source_alerts: scan failed")
			continue
		}
		...
		if authorHandle != nil {
			...
			item["author_handle"] = "@211"
		}
		if authorDisplay != nil {
			item["author_display_name"] = *authorDisplay
		} else {
			item["author_display_name"] = "211 Resources"
		}
		item["author_id"] = "00000000-0000-0000-0000-000000000211"
		if imageURL != nil {
			item["image_url"] = *imageURL
		}
		if expiresAt != nil {
