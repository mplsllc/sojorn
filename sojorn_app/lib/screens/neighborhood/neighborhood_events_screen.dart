...
/// Events screen for a neighborhood — wraps the group events API
/// using the neighborhood's underlying group_id.
class NeighborhoodEventsScreen extends StatefulWidget {
  final String groupId;
  final String neighborhoodName;
  final String? userRole; // 'admin', 'moderator', 'resident'

  const NeighborhoodEventsScreen({
    super.key,
  ...
  @override
  State<NeighborhoodEventsScreen> createState() =>
      _NeighborhoodEventsScreenState();
}

class _NeighborhoodEventsScreenState extends State<NeighborhoodEventsScreen> {
  ...
  Future<void> _loadEvents() async {
    ...
    try {
      ...
      debugPrint('[NeighborhoodEvents] Error: $e');
    }
  ...
  Future<void> _rsvp(GroupEvent event, RSVPStatus status) async {
    try {
      await ApiService.instance
          .rsvpEvent(widget.groupId, event.id, status.value);
      _loadEvents();
    } catch (e) {
  ...
  void _showCreateEvent() {
    showModalBottomSheet(
      ...
      shape: const RoundedRectangleBorder(
        borderRadius:
            BorderRadius.vertical(top: Radius.circular(SojornRadii.modal)),
      ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('${widget.neighborhoodName} Events'),
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        if (_canCreate && index == 0) {
                          return Padding(
                            padding:
                                const EdgeInsets.only(bottom: SojornSpacing.md),
                        ...
                        final event =
                            _events[_canCreate ? index - 1 : index];
                        return _EventCard(
                          event: event,
                          groupId: widget.groupId,
                          isAdmin: _canCreate,
              ...
              Row(
                children: [
                  Icon(Icons.calendar_today,
                      size: 14, color: SojornColors.basicRoyalPurple),
      ...
      child: Container(
        ...
        child: Text(
          ...
          style: TextStyle(
            ...
            fontWeight: FontWeight.w600,
            color:
                isSelected ? Colors.white : SojornColors.basicRoyalPurple,
          ),
        ),
      ),
    );
  ...
  Future<void> _pickDate(bool isEnd) async {
    ...
    final time = await showTimePicker(
      ...
      initialTime: TimeOfDay.fromDateTime(
          isEnd ? (_endsAt ?? _startsAt) : _startsAt),
    );
  ...
  Future<void> _submit() async {
    ...
    try {
      await ApiService.instance.createGroupEvent(widget.groupId, {
        ...
        'location_name': _locationCtrl.text.trim().isEmpty
            ? null
        ...
        child: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              subtitle: Text(
                  _endsAt != null ? dateFmt.format(_endsAt!) : 'Tap to set'),
            ...
            SwitchListTile(
              ...
              subtitle:
                  const Text('Visible to users outside this neighborhood'),
              value: _isPublic,
